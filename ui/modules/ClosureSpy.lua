...
scriptContext:SetCallback(function()
    local script = "-- This script was generated by Hydroxide's ClosureSpy: https://github.com/Upbolt/Hydroxide\n\n"
    local selectedHook = selected.hookLog.Hook
    local closureName = selectedHook.Closure.Name
    local callingScriptPath = (selected.callingScript and getInstancePath(selected.callingScript)) or "N/A"

    script = script .. ("-- Closure Name: %s\n"):format(closureName)
    script = script .. ("-- Calling Script: %s\n\n"):format(callingScriptPath)
